...
class ArucoJengaBlockDetector(Node):
    def __init__(self):
        ...
        self.fov_x_mm = 660  # Width of FOV in mm
        self.fov_y_mm = 480  # Height of FOV in mm
        ...
        self.pose_pub = self.create_publisher(PoseArray, '/jenga_detection_poses', 10)
    ...
    def jenga_callback(self, msg):
        ...
        poses = self.detect_jenga_block(cv_image, aruco_markers)  # Collect all poses

        # Publish all poses together if any are detected
        if poses:
            pose_array_msg = PoseArray()
            pose_array_msg.header.stamp = self.get_clock().now().to_msg()
            pose_array_msg.header.frame_id = "camera_link"  # Change to your actual frame ID
            pose_array_msg.poses = poses  # Assign all detected poses

            self.pose_pub.publish(pose_array_msg)  # Publish the PoseArray
    ...
    def detect_jenga_block(self, cv_image, aruco_markers):
        ...
        jenga_data_list = []
        self.poses = []  # Create an empty list to collect poses
        ...
        return self.poses  # Return all collected poses    
    ...
    def draw_oriented_bounding_box(self, cv_image, obb_xyxyxyxy, aruco_markers):
        ...
        for marker_id, (aruco_x, aruco_y) in aruco_markers.items():
            if (obb_corners[:, 0].min() <= aruco_x <= obb_corners[:, 0].max() and
                ...
                dx = obb_corners[2][1] - obb_corners[1][1]
                dy = obb_corners[2][0] - obb_corners[1][0]
                ...
                label_with_coords = f"{label} ({real_y_mm:.2f}mm, {real_x_mm:.2f}mm, {theta_degrees:.2f} deg)"
                ...
                pose = self.make_pose_message(real_y_mm, real_x_mm, theta_degrees)
                self.poses.append(pose)  # Append each pose to the list

                return label_with_coords  # Only return the label string
